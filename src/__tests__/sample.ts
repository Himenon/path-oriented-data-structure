...
export const createFindComponent = (operator: Operator<string>) => <T extends Kind>(kind: T, path: string): GetNode<T> | undefined => {
  return operator.getComponent(kind, path) as GetNode<T> | undefined;
};
